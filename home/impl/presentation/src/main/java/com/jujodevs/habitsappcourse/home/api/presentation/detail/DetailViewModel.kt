...
import com.jujodevs.habitsappcourse.home.api.domain.detail.usecase.DetailUseCases
...
class DetailViewModel @Inject constructor(
    ...
    init {
        val id = savedStateHandle.get<String>(HABIT_ID_KEY)
        if (id != null) {
    ...
    fun onEvent(event: DetailEvent) {
        when (event) {
            is DetailEvent.FrequencyChange -> {
                val frequency =
                    if (state.frequency.contains(event.dayOfWeek)) {
